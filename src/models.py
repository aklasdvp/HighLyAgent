...
class KnowledgeEntry(Base):
    ...
    embedding = mapped_column(Vector(settings.VECTOR_DIM))
...
class User(Base):
    """End-user of a client application (multi-user per client)."""
    __tablename__ = "users"
...
class Conversation(Base):
    ...
    id: Mapped[_uuid_mod.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=_uuid)
    client_id: Mapped[_uuid_mod.UUID] = mapped_column(ForeignKey("clients.id", ondelete="CASCADE"))
...
class Message(Base):
    ...
    source: Mapped[str | None] = mapped_column(String(16))                     # knowledge|ai|tool
