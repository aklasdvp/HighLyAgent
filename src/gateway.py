...
class ConnectionManager:
    def __init__(self):
        ...
        self.tasks: dict[str, CancelToken] = {}
    ...
    async def dispatch_client_tool(self, scope: str, tool_name: str, args: dict, task_id: str) -> Any:
        fut: asyncio.Future = asyncio.get_running_loop().create_future()
        self.pending_tools[f"{task_id}:{tool_name}"] = fut
        for conn_id in self.by_client.get(uuid.UUID(scope), set()):
            await self.send(conn_id, {"type": "tool_request", "task_id": task_id, "tool_name": tool_name, "args": args})
        return await fut
...
async def _authenticate(token: str | None) -> tuple[uuid.UUID | None, str]:
    """Authenticate either a dashboard JWT or a project API key."""
    if not token:
        return None, "anon"
...
async def websocket_endpoint(
    ...
    """Real-time gateway with explicit API-key support.

    Preferred project-client format is ``?client_id=<id>&api_key=<key>``. The
    legacy ``token=<key>`` format remains supported, while dashboard JWT users
    continue to use ``token=<jwt>``. A project/API-key mismatch is rejected
    before the connection is registered.
    """
    await ws.accept()

    credential = api_key or token
    key_client, auth_kind = await _authenticate(credential)

    # API-key requests must name the same project the key belongs to. Supplying
    # an invalid key or mismatched project is an authorization failure, not an
    # anonymous connection.
    if api_key and auth_kind != "api-key":
        await ws.close(code=4403, reason="ACCESS_DENIED: invalid API key")
        return
    if auth_kind == "api-key":
        if client_id is None:
            await ws.close(code=4400, reason="BAD_REQUEST: client_id is required with API key")
            return
        if str(key_client) != client_id:
    ...
    try:
        resolved_client_id = uuid.UUID(client_id) if client_id else key_client
    except ValueError:
        await ws.close(code=4400, reason="BAD_REQUEST: invalid client_id")
        return

    conn_id = str(uuid.uuid4())
    manager.register(conn_id, ws, resolved_client_id)
    log.info("ws connect conn=%s auth=%s client=%s", conn_id, auth_kind, resolved_client_id)
    ...
    try:
        while True:
            ...
            if ftype == "chat":
                ...
                asyncio.create_task(_run_task(conn_id, resolved_client_id, task_id, frame.get("text", "")))
...
async def _run_task(conn_id: str, client_id: uuid.UUID | None, task_id: str, text: str):
    ...
    async def emit(stage: str, pct: int, detail: str | None = None):
        await manager.send(conn_id, {"type": "progress", "task_id": task_id, "stage": stage, "pct": pct, "detail": detail})
    ...
    try:
        async with async_session() as db:
            ...
            user = (await db.execute(select(User).where(User.client_id == client.id).limit(1))).scalar_one_or_none()
            conv = Conversation(user_id=user.id)
            db.add(conv)
            await db.commit()
            agent = AgentCore(db, KnowledgeEngine(db))
            registry.set_client_dispatcher(lambda scope, name, args: manager.dispatch_client_tool(scope, name, args, task_id))
