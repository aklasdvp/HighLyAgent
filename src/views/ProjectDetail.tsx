...
export default function ProjectDetail({ projectId, onBack, onDeleted }: {
  ...
  return (
    <div>
      ...
      <div className="mt-5">
        ...
        {tab === 'settings' && <SettingsTab client={client} onDeleted={onDeleted} />}
      </div>
    </div>
  );
  ...
  function AiTab({ client }: { client: ClientApp }) {
    const { state, actions } = useStore();
    const { push } = useToast();
    const [form, setForm] = useState<AIClientConfig>(client.aiConfig ?? { ...DEFAULT_AI_CONFIG });
              ...
              <div key={e.id} className="px-4 py-3 hover:bg-ink-750/50 transition-colors">
                <div className="flex items-start gap-3">
                  <div className="min-w-0 grow">
                    ...
                    <p className="text-[12px] text-mist-400 mt-1 line-clamp-2 leading-relaxed">{e.answer}</p>
  ...
  function UsageTab({ client }: { client: ClientApp }) {
    const { state, actions } = useStore();
    const { push } = useToast();
    const [rpm, setRpm] = useState(client.rateLimitPerMin ?? 60);
    const users = state.users.filter((u) => u.clientId === client.id);
      ...
      <div className="grid lg:grid-cols-2 gap-3 items-start">
        <div className="panel p-5 anim-rise">
          ...
          <div className="flex justify-end gap-2">
            <Btn disabled={rpm === (client.rateLimitPerMin ?? 60)} onClick={() => setRpm(client.rateLimitPerMin ?? 60)}>Discard</Btn>
            <Btn variant="primary" disabled={rpm === (client.rateLimitPerMin ?? 60)} onClick={() => { actions.updateClient(client.id, { rateLimitPerMin: rpm }); push(`Rate limit saved — ${rpm} req/min`); }}>
      ...
      <div className="grid lg:grid-cols-3 gap-3 items-start">
        <div className="panel p-5 lg:col-span-2 anim-rise">
          ...
          <div className="sticky bottom-0 z-10 -mx-5 -mb-5 px-5 py-3 border-t border-ink-700 bg-ink-800/95 backdrop-blur flex items-center justify-between rounded-b-[10px]">
            ...
            <div className="flex gap-2">
              <Btn size="sm" disabled={!dirty} onClick={() => setForm({ name: client.name, desc: client.desc, type: client.type, env: client.env, allowedOrigins: client.allowedOrigins ?? '', webhookUrl: client.webhookUrl ?? '' })}>Discard</Btn>
